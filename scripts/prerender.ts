...
const langMeta: Record<Lang, { htmlLang: string; locale: string; siteTitle: string }> = {
  es: { htmlLang: 'es', locale: 'es_CO', siteTitle: 'CCGrupo | BPO Colombia - CX, Ventas, IA y Digital Studio' },
  en: { htmlLang: 'en', locale: 'en_US', siteTitle: 'CCGrupo | BPO Colombia - CX, Sales, AI and Digital Studio' },
  pt: { htmlLang: 'pt', locale: 'pt_BR', siteTitle: 'CCGrupo | BPO Colombia - CX, Vendas, IA e Digital Studio' },
};
...
function toUrl(route: string): string {
  ...
  return `https://ccgrupo.com.co${normalized}`;
}
...
function renderHome(lang: Lang): string {
  ...
  return shell(`
      ...
      ${section(t.footer.contactTitle, `
        <p style="margin:0 0 8px">${escapeHtml('Cra. 20 #133 - 74, La Calleja')}</p>
        <p style="margin:0 0 8px">${escapeHtml('info@ccgrupo.com.co')}</p>
        <p style="margin:0">${escapeHtml('(601) 7443732')}</p>
      `)}
  `);
}
...
function renderContact(lang: Lang): string {
  ...
  return shell(`
      ...
      ${section(lang === 'en' ? 'Contact details' : lang === 'pt' ? 'Contato' : 'Contacto', list([
        ...
        `${t.contact.infoLabels.email}: info@ccgrupo.com.co`,
        `${t.contact.infoLabels.phone}: (601) 7443732`,
...
function pageAlternates(route: string, lang: Lang): Array<{ hreflang: string; href: string }> {
  const base = toUrl(route);
  if (route === '/') {
    return [
      { hreflang: 'es-co', href: 'https://ccgrupo.com.co/' },
      { hreflang: 'en', href: 'https://ccgrupo.com.co/en' },
      { hreflang: 'pt', href: 'https://ccgrupo.com.co/pt' },
      { hreflang: 'x-default', href: 'https://ccgrupo.com.co/' },
    ];
  }
  ...
  if (home.startsWith('servicio/')) {
    ...
    return [
      { hreflang: 'es-co', href: `https://ccgrupo.com.co/servicio/${id}` },
      { hreflang: 'en', href: `https://ccgrupo.com.co/en/servicio/${id}` },
      { hreflang: 'x-default', href: `https://ccgrupo.com.co/servicio/${id}` },
    ];
  }

  if (home === 'contacto' || home === 'contact') {
    return [];
  }

  if (home === 'politicas-privacidad' || home === 'privacy' || home === 'privacy-policies') {
    return [];
  }
...
async function main() {
  ...
  for (const lang of ['es', 'en', 'pt'] as const) {
    ...
    pages.push({
      ...
      lang,
      title: `${t.privacy.heading} | CCGrupo`,
